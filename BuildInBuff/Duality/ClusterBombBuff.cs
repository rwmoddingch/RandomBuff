...
namespace BuiltinBuffs.Duality
{
    internal class ClusterBombBuffEntry : IBuffEntry
    {
        ...
        private static void Explosion_ctor(On.Explosion.orig_ctor orig, Explosion self, Room room, PhysicalObject sourceObject, Vector2 pos, int lifeTime, float rad, float force, float damage, float stun, float deafen, Creature killTagHolder, float killTagHolderDmgFactor, float minStun, float backgroundNoise)
        {
            ...
            if(damage > 1f)
            {
                int count = (int)Mathf.Clamp(damage * 4, 2, 40);
        ...
        public static void CreateClusterBombs(Room room, int count, PhysicalObject sourceObject, Vector2 pos, int lifeTime, float rad, float force, float damage, float stun, float deafen, Creature killTagHolder, float killTagHolderDmgFactor, float minStun, float backgroundNoise)
        {
            ...
            emitter.ApplyParticleModule(new AddElement(emitter, new Particle.SpriteInitParam("pixel", "", 5)));
            ...
            emitter.ApplyParticleModule(new SetRandomScale(emitter, 4f, 8f));
            emitter.ApplyParticleModule(new SetRandomLife(emitter, 20, 120));
            emitter.ApplyParticleModule(new SetSphericalVelocity(emitter, force * 25, force * 25 + 2f));
            emitter.ApplyParticleModule(new ConstantAcc(emitter, new Vector2(0f, -25f)));
            ...
            {
                room.AddObject(new SootMark(room, p.pos, 80f * damage, true));
                ...
                for (int i = 0; i < 25; i++)
                {
                    ...
                    room.AddObject(new Explosion.FlashingSmoke(p.pos + a * 40f * Random.value, a * Mathf.Lerp(4f, 20f, Mathf.Pow(Random.value, 2f)), 1f + 0.05f * Random.value, new Color(1f, 1f, 1f), Color.black, Random.Range(3, 11)));
                }
