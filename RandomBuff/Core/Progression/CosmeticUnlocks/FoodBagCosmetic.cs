...
namespace RandomBuff.Core.Progression.CosmeticUnlocks
{
    ...
    {
        public static Queue<FakeFood> fFoodPool = new Queue<FakeFood>();
        ...
        public static FakeFood GetFood(Room room, Vector2 initPos, Vector2 initVel)
        {
            ...
            else
            {
                ...
                FakeFood fakeFood = fFoodPool.Dequeue();
                fakeFood.room = room;
                ...
                fakeFood.foodType = Mathf.Clamp(UnityEngine.Random.Range(1,6),1,5);
    ...
    public class FakeFood : UpdatableAndDeletable, IDrawable
    {
        ...
        public float[] getRad = new float[5]
        {
            8f,//DangleFruit
            6f,//Lantern
            5f,//SlimeMold
        ...
        public float[] getBuoyancy = new float[5]
        {
            1.1f,//DangleFruit
            0.8f,//Lantern
            1.1f,//SlimeMold
        ...
        public FakeFood(Room room, int foodType, Vector2 initPos, Vector2 initVel)
        {
            ...
            this.active = true;
            this.room = room;
            this.foodType = foodType > 5 ? 5 : foodType;
            this.pos = initPos;
            this.vel = initVel;
        }
        ...
        public override void Update(bool eu)
        {
            base.Update(eu);
            if (!active) return;
            if (this.room == null || !this.room.ViewedByAnyCamera(this.pos, 10f))
            {
            ...
            for (int i = 0; i < 9; i++)
            {
                if (room.GetTile(room.GetTilePosition(this.pos) + Custom.eightDirectionsAndZero[i]).Terrain == Room.Tile.TerrainType.Solid)
                {
                    ...
                    if (this.pos.y >= vector.y - 10f && this.pos.y <= vector.y + 10f)
                    {
                        ...
                        {
                            if (this.pos.x > vector.x - 10f - this.getRad[foodType-1] && room.GetTile(room.GetTilePosition(this.pos) + Custom.eightDirectionsAndZero[i] + new IntVector2(-1, 0)).Terrain != Room.Tile.TerrainType.Solid)
                            {
                                num = vector.x - 10f - this.getRad[foodType-1];
                            }
                        }
                        else if (this.pos.x < vector.x + 10f + this.getRad[foodType-1] && room.GetTile(room.GetTilePosition(this.pos) + Custom.eightDirectionsAndZero[i] + new IntVector2(1, 0)).Terrain != Room.Tile.TerrainType.Solid)
                        {
                            num = vector.x + 10f + this.getRad[foodType-1];
                        }
                    }
                    if (this.pos.x >= vector.x - 10f && this.pos.x <= vector.x + 10f)
                    {
                        ...
                        {
                            if (this.pos.y > vector.y - 10f - this.getRad[foodType-1] && room.GetTile(room.GetTilePosition(this.pos) + Custom.eightDirectionsAndZero[i] + new IntVector2(0, -1)).Terrain != Room.Tile.TerrainType.Solid)
                            {
                                num2 = vector.y - 10f - this.getRad[foodType-1];
                            }
                        }
                        else if (this.pos.y < vector.y + 10f + this.getRad[foodType-1] && room.GetTile(room.GetTilePosition(this.pos) + Custom.eightDirectionsAndZero[i] + new IntVector2(0, 1)).Terrain != Room.Tile.TerrainType.Solid)
                        {
                            num2 = vector.y + 10f + this.getRad[foodType-1];
                        }
                    }
                    ...
                    else
                    {
                        ...
                        if (Custom.DistLess(this.pos, vector2, this.getRad[foodType-1]))
                        {
                            float num3 = Vector2.Distance(this.pos, vector2);
                            Vector2 a = Custom.DirVec(this.pos, vector2);
                            this.vel *= this.surfaceFric;
                            this.pos -= (this.getRad[foodType-1] - num3) * a;
                            this.vel -= (this.getRad[foodType-1] - num3) * a;
                            this.terrainContact = true;
                        }
                    }
                }
                else if (Custom.eightDirectionsAndZero[i].x == 0 && room.GetTile(room.GetTilePosition(this.pos) + Custom.eightDirectionsAndZero[i]).Terrain == Room.Tile.TerrainType.Slope)
                {
                    ...
                    {
                        if (this.pos.y < vector3.y - (vector3.x - this.pos.x) + this.getRad[foodType-1])
                        {
                            this.pos.y = vector3.y - (vector3.x - this.pos.x) + this.getRad[foodType-1];
                            this.vel.y = 0f;
                            this.vel.x = this.vel.x * this.surfaceFric;
                            this.terrainContact = true;
                        }
                    ...
                    else if (room.IdentifySlope(room.GetTilePosition(this.pos) + Custom.eightDirectionsAndZero[i]) == Room.SlopeDirection.DownRight && this.pos.y > vector3.y - (vector3.x - this.pos.x) - this.getRad[foodType-1])
                    {
                        this.pos.y = vector3.y - (vector3.x - this.pos.x) - this.getRad[foodType-1];
                        this.vel.y = 0f;
                        this.vel.x = this.vel.x * this.surfaceFric;
